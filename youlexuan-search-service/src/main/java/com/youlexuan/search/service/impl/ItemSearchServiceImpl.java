...
public class ItemSearchServiceImpl implements ItemSearchService {
    ...
    private Map searchBrandAndSpecList(String category){
        Map map=new HashMap();
        Long typeId = (Long) redisTemplate.boundHashOps("itemcat_list_key").get(category);//获取模板ID
        if(typeId!=null){
            //根据模板ID查询品牌列表
            List brandList = (List) redisTemplate.boundHashOps("brand_list_key").get(typeId);
            map.put("brandList", brandList);//返回值添加品牌列表
            //根据模板ID查询规格列表
            List specList = (List) redisTemplate.boundHashOps("spec_list_key").get(typeId);
    ...
    private Map searchList(Map searchMap){
        ...
        if(pageSize==null) pageSize=10; //默认每页10条
